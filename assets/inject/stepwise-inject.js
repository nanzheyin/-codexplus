(() => {
  ...
  function toggleCodexTheme() {
    ...
    setCodexThemeMode(nextTheme)
      ...
      .catch((error) => {
        console.warn("[Codex Deck Stepwise] Failed to switch Codex theme", error);
      });
  }
  ...
  function statusLine(settings) {
    if (settings.enabled !== true) return "Stepwise 已关闭，请在 Codex Deck 管理工具里开启。";
  ...
  async function openManager() {
    state.settingsStatus = "正在打开 Codex Deck 管理工具...";
    renderFloat();
    const payload = await bridgeCall("/manager/open", {});
    if (!isCurrentInstance()) return;
    state.settingsStatus = payload?.status === "ok" ? "已打开 Codex Deck 管理工具" : payload?.message || "打开失败";
    renderFloat();
  }
